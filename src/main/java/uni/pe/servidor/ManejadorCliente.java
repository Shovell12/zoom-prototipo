...
public class ManejadorCliente implements Runnable {
    ...
    private ManejadorCliente(Socket socket) {
        this.socket = socket;
        registrarComandos();
    }

    static ManejadorCliente crear(Socket socket) {
        return new ManejadorCliente(socket);
    }
    ...
    private void handleLogin(MensajeSocket msg) {
        Usuario u = service.login(msg.getCorreo(), msg.getPassword());
    ...
    private void handleRegistro(MensajeSocket msg) {
        boolean ok = service.registrar(msg.getNombres(), msg.getCorreo(), msg.getPassword());
    ...
    private void handleCrearSala(MensajeSocket msg) {
        String codigo = service.crearSala(msg.getRoomCode(), msg.getRoomName(), idUsuario);
        if (codigo != null) {
            this.roomCode = codigo;
            Servidor.unirseASala(codigo, idUsuario);
            enviar(new MensajeSocket.Builder(MensajeSocket.CREATE_ROOM_RESPONSE)
                    .sala(codigo, msg.getRoomName()).respuesta(true, "Sala creada").build());
        } else {
            enviar(new MensajeSocket.Builder(MensajeSocket.CREATE_ROOM_RESPONSE)
                    .respuesta(false, "Error al crear sala").build());
        }
    }

    private void handleUnirSala(MensajeSocket msg) {
        int idHost = service.solicitarUnirse(msg.getRoomCode(), idUsuario);
        if (idHost < 0) { enviar(error("Sala no encontrada")); return; }
        this.roomCode = msg.getRoomCode();
        Servidor.enviarA(idHost, new MensajeSocket.Builder(MensajeSocket.WAITING_ROOM_UPDATE)
    ...
    private void handleAdmitir(MensajeSocket msg) {
        if (msg.isAceptado()) {
            service.admitirEnSala(msg.getRoomCode(), msg.getIdUsuario());
    ...
    private void handleSalir(MensajeSocket msg) {
        ...
        if (service.esHost(roomCode, idUsuario)) {
    ...
    private MensajeSocket error(String txt) {
        return new MensajeSocket.Builder(MensajeSocket.ERROR).texto(txt).build();
    }
